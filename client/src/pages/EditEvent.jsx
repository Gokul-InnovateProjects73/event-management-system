    ...
    useEffect(() => {
        const fetchEvent = async () => {
            try {
                ...
                // Only organizer or admin may edit
                if (String(user?._id) !== String(ev.organizer?._id) && !isAdmin) {
                ...
                if (ev.ticketTypes && ev.ticketTypes.length > 0) {
                    setTicketTypes(ev.ticketTypes.map(t => ({ name: t.name, price: t.price, capacity: t.capacity })));
                }
                if (ev.agenda && ev.agenda.length > 0) {
                    setAgenda(ev.agenda.map(a => ({ time: a.time || '', title: a.title || '', description: a.description || '' })));
                }
            } catch {
